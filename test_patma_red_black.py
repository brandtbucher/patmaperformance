...
import bm_patma_red_black


def assert_valid_tree(node: bm_patma_red_black.Node | None) -> int:
    ...
    match node:
        case None:
            ...
            return 1
        case (
            bm_patma_red_black.Red(left=bm_patma_red_black.Red())
            | bm_patma_red_black.Red(right=bm_patma_red_black.Red())
        ):
            assert False, f"children of Red nodes must be Black (got {node})"
        case (
            bm_patma_red_black.Node(ge, left=bm_patma_red_black.Node(le))
            | bm_patma_red_black.Node(le, right=bm_patma_red_black.Node(ge))
        ) if ge < le:
    ...
    return depth + isinstance(node, bm_patma_red_black.Black)


@hypothesis.given(values=hypothesis.infer)
def test_patma_red_black(values: list[int]) -> None:
    hypothesis.assume(1 <= len(values) < 1 << 8)
    root = bm_patma_red_black.Black(values.pop(), None, None)
    for value in values:
        root = root.add(value)
